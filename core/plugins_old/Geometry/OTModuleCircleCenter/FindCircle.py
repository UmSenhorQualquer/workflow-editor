...
class FindCircle(object):
    ...
    def find_radius(self, img, y_start=0, y_end=None, fase=0, blockSize=1):
        """
    ...
    def find_circle(self, image):
        ...
        polar_grid, r, theta = reproject_image_into_polar(image, origin=pos)
        border = np.zeros_like( image, dtype=np.uint8 ); cv2.rectangle( border, (4,4), (border.shape[1]-5,border.shape[0]-5), (255,255,255), -1 )
        polar_border, r, theta = reproject_image_into_polar(border, origin=pos)
        clean_grad_y = remove_vertical_objects(polar_grid, polar_border)

        y_r = self.find_radius(clean_grad_y)
